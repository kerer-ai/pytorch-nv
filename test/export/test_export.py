...
class TestExportFlexAttention(TestCase):
    ...
    def test_flex_attention_export(self, device):
        ...
        try:
            _validate_device(probe, probe, probe)
        except (ValueError, NotImplementedError) as err:
            self.skipTest(str(err))
        if torch.device(device).type in ("cuda", "xpu") and not has_triton():
            self.skipTest("FlexAttention GPU path requires triton")
...
_DEVICE_EXPORT_TEST_SPECS = (
    (TestExportCuda, {"only_for": "cuda"}),
    (TestExportFlexAttention, {"allow_xpu": True, "allow_mps": True}),
    (TestExportAccelerator, {"except_for": "cpu", "allow_xpu": True}),
    (TestExportRNN, {"except_for": "cpu", "allow_xpu": True}),
    (TestExportTriton, {"except_for": "cpu", "allow_xpu": True}),
    (TestExportMoveToDeviceIndex, {"except_for": "cpu", "allow_xpu": True}),
    (TestExportAutocastException, {"except_for": "cpu", "allow_xpu": True}),
)
